...
__device__ inline GpuFloat3 random_unit_sphere_direction(RngState* rng) {
    float r1 = rng_uniform(rng);
    float r2 = rng_uniform(rng);
...
__device__ inline Onb onb_from_normal(GpuFloat3 n) {
    Onb onb;
    onb.w = n;
    GpuFloat3 a = fabsf(n.x) > 0.9f ? GpuFloat3{0.0f, 1.0f, 0.0f} : GpuFloat3{1.0f, 0.0f, 0.0f};
    onb.v = vec_normalize(vec_cross(n, a));
    onb.u = vec_cross(n, onb.v);
    return onb;
}

__device__ inline GpuFloat3 onb_transform(const Onb* onb, GpuFloat3 v) {
    return vec_add(
...
__device__ inline ScatterResult scatter_lambertian(
    GpuFloat3 normal, GpuFloat3 albedo, RngState* rng)
{
    ScatterResult sr;
...
__device__ inline ScatterResult scatter_metal(
    GpuFloat3 ray_dir, GpuFloat3 normal, GpuFloat3 albedo, float fuzz, RngState* rng)
{
    ScatterResult sr;
    sr.attenuation = albedo;
...
__device__ inline ScatterResult scatter_dielectric(
    GpuFloat3 ray_dir, GpuFloat3 normal, float ir, bool front_face, RngState* rng)
{
    ScatterResult sr;
    sr.attenuation = {1.0f, 1.0f, 1.0f};
    float refraction_ratio = front_face ? (1.0f / ir) : ir;

    GpuFloat3 unit_dir = vec_normalize(ray_dir);
    float cos_theta = fminf(vec_dot({-unit_dir.x, -unit_dir.y, -unit_dir.z}, normal), 1.0f);
